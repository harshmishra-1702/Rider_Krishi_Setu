...
class _OtpDeliveryScreenState extends ConsumerState<OtpDeliveryScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final canEnterOtp = allBagsVerified && photoAttached;
    ...
    return Scaffold(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              if (!allBagsVerified) {
                ttsService.speak(
                  'Please perform delivery verification scan. Scan or enter the batch ID for all ${trip.waypoints.length} produce bags before entering the OTP.',
                  languageCode: selectedLang.ttsLocale,
                );
              } else if (!photoAttached) {
                ttsService.speak(
                  'Bags verified. Please snap a produce photo of unloaded crates at the mandi.',
                  languageCode: selectedLang.ttsLocale,
                );
              } else {
                ttsService.speak(
                  'Bags and photo verified. Ask mandi supervisor for their 4-digit handover OTP.',
                  languageCode: selectedLang.ttsLocale,
                );
              }
            },
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // ── Geofence Check Card ─────────────────────────────────
            _buildGeofenceCard(deliveryState, trip.destination.name, strings),
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    label: const Text('Enter Batch #', style: TextStyle(fontSize: 12)),
                  ),
                ),
                  ...
                  child: ElevatedButton.icon(
                    onPressed: allBagsVerified
                        ...
                        : () {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text('✓ All packages verified for this destination!'),
                                backgroundColor: AppColors.statusDelivered,
                              ),
                            );
                          },
                    icon: const Icon(Icons.checklist_rtl, size: 18),
                    label: const Text('Verify All Bags', style: TextStyle(fontSize: 12)),
                  ...
                  Container(
                    ...
                    child: const Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 4),
                        Text('Locked', style: TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                      ],
                    ),
                  ),
              ],
            ...
            const SizedBox(height: 4),
            const Text(
              'Photograph produce safely unloaded at mandi receiving dock / pre-cooling unit.',
              style: TextStyle(fontSize: 12, color: AppColors.textSecondary),
            ),
            const SizedBox(height: 8),
            ...
            // ── Step 3: Buyer Handover OTP Card ─────────────────────
            Row(
              children: [
                Expanded(
                  child: Text(
                    'Step 3: Mandi / Buyer Handover OTP',
                    style: Theme.of(context).textTheme.titleLarge,
                  ),
                ),
                IconButton(
                  ...
                  onPressed: () {
                    ttsService.speak(
                      'Ask buyer or mandi supervisor for their 4-digit handover OTP.',
                      languageCode: selectedLang.ttsLocale,
                    );
                  },
                ),
            ...
            Text(
              canEnterOtp
                  ? 'Ask Mandi Receiving Supervisor / Buyer for 4-digit OTP to release instant payout.'
                  : '🔒 Locked: Verify all unloaded bags (Step 1) and attach produce photo (Step 2) to unlock OTP entry.',
              style: TextStyle(
                fontSize: 12,
                color: canEnterOtp ? AppColors.textSecondary : AppColors.danger,
                fontWeight: canEnterOtp ? FontWeight.normal : FontWeight.w600,
              ),
            ),
            const SizedBox(height: 16),
            ...
            ElevatedButton(
              onPressed: !canEnterOtp ||
                  ...
                  : () async {
                      ...
                      if (success && mounted) {
                        ...
                        ttsService.speak(
                          'Trip delivered successfully. Payout has been disbursed to your escrow wallet.',
                          languageCode: selectedLang.ttsLocale,
                        );
                        context.go('/earnings');
                      }
              ...
              child: deliveryState.isSubmittingOtp
                  ...
                  : const Text('Confirm Delivery & Disburse Payout'),
            ),
            ...
            if (canEnterOtp)
              Center(
                child: TextButton.icon(
                  onPressed: () {
                    // Demo helper: Fill mock 4-digit OTP '5678'
                    for (int i = 0; i < 4; i++) {
                      _otpControllers[i].text = '5678'[i];
                    }
                    setState(() {});
                  },
                  icon: const Icon(Icons.key, size: 18),
                  label: const Text('Auto-fill Test OTP (5678)'),
                ),
              ),
          ],
        ),
      ...
      children: [
                ...
                : Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text(
                          'Mandi receiving dock produce snapshot',
                          style: TextStyle(
        ...
        if (isUnlocked && photoPath == null) ...[
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              onPressed: () {
                ref.read(deliveryProvider.notifier).setPhotoPath('mock://mandi_dock_crates_verified.jpg');
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('✓ Sample produce photo attached (Mandi Receiving Dock proof).'),
                    backgroundColor: AppColors.statusDelivered,
                  ),
                );
              },
